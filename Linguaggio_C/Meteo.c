...
struct archivio registra(struct archivio a, struct dati_meteo d) registra un nuovo insieme di dati meteo d nell'archivio a e restituisce 
l'archivio aggiornato.
Attenzione: Ricordatevi che le modifiche efiettuate dalla funzione sull'archivio non sono visibili all'esterno, quindi l'archivio
aggiornato va restituito come risultato della funzione e assegnato alla variabile precedente.
struct statistiche calcola(struct archivio a) ritorna una struttura contenente le statistiche dei dati inseriti. La struttura struct 
statistiche contiene i valori minimi/massimi/medi di temperatura/pressione/umidità/pioggia.
...
int main(){
    ...
    do{
        ...
        switch (scelta){  //in base a scelta faccio cose:
            ...
            case ('b'): 
                ...
                struct statistiche stat = calcola(arch);

                printf("medie: temperatura: %d gradi\npressione: %d bar\numidita:%d percento\npioggia: %d mm\n\n",stat.t.mid,stat.pr.mid,stat.u.mid,stat.pi.mid);
            break;
    ...
    }while(scelta=='a' || scelta=='b'); //se ho scelto c, esco dal programma;
}
...
struct archivio registra(struct archivio a, struct dati_meteo dato){
    ...
    a.numerodati++;
    printf("dato salvato! dati inseriti: %d\n",a.numerodati);
    return a;
}

struct statistiche calcola(struct archivio a){
    struct statistiche s;
    int i,sommapioggia=0,sommatemp=0,sommapress=0,sommaumidita=0;
    for(i=0;i<a.numerodati;i++){
        sommapioggia = sommapioggia + a.dati[i].piog;
        sommatemp = sommatemp + a.dati[i].temp;
        sommapress = sommapress + a.dati[i].press;
        sommaumidita = sommaumidita + a.dati[i].umid;

    }
    s.pi.mid = sommapioggia / a.numerodati;
    s.t.mid = sommatemp / a.numerodati;
    s.pr.mid = sommapress / a.numerodati;
    s.u.mid = sommaumidita / a.numerodati;
